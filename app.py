...
def fmt_time(val) -> str:
    """
    Normalise any time-like value (timedelta, datetime, or string) to "HH:MM".
...
def calc_duration(checkin: str, checkout: str) -> str:
    ...
    Returns "" if either argument is missing or unparseable.
    """
...
def _split_tokens(text: str) -> list[str]:
    ...
    """
    cleaned = re.sub(r"[+&/\-]", " ", _normalise(text))
...
def validate_workbook(xl: pd.ExcelFile) -> list[str]:
    ...
    # 2. Verify column presence (strip trailing spaces from headers)
    col_checks = [
    ...
    for sheet_name, required_cols in col_checks:
        ...
        for col in required_cols:
            if col not in actual:
                errors.append(f"Sheet '{sheet_name}' — missing column: '{col}'")
...
def reconcile(xl: pd.ExcelFile) -> dict:
    ...
    # ── 1. Load sheets ────────────────────────────────────────────────────────
    visitor_df = pd.read_excel(xl, sheet_name=SHEET_VISITOR)
    card_df    = pd.read_excel(xl, sheet_name=SHEET_CARD)
    access_df  = pd.read_excel(xl, sheet_name=SHEET_ACCESS)

    # Strip trailing whitespace from all column headers
    for df in (visitor_df, card_df, access_df):
    ...
    card_df = card_df[
        ...
        card_df[CA_CARDNUM].notna() &
        (card_df[CA_CARDNUM].astype(str).str.strip().str.isnumeric())
    ].copy()
    card_df[CA_CARDNUM] = card_df[CA_CARDNUM].astype(int)

    # Pre-compute a normalised date key on each sheet for fast lookup
    # (converts Timestamps, strings, etc. to a uniform lowercase string)
    card_df["_date_key"]   = card_df[CA_DATE].apply(
        lambda v: _normalise(str(pd.Timestamp(v).date())
                             if not isinstance(v, str) else v)
    )
    access_df["_date_key"] = access_df[AL_DATE].apply(
        lambda v: _normalise(str(pd.Timestamp(v).date())
                             if not isinstance(v, str) else v)
    )
    ...
    for idx, vrow in visitor_df.iterrows():
        visitor_name = str(vrow[VR_VISITOR]).strip()
        visit_date   = vrow[VR_DATE]

        # Normalise visit_date to a consistent string key
        try:
            date_key = _normalise(str(pd.Timestamp(visit_date).date()))
        except Exception:
            date_key = _normalise(str(visit_date))
    ...
    # ── Unreturned cards ──────────────────────────────────────────────────────
    # "B" = borrowed/not-returned in this workbook; blank/NaN also = not returned
    unreturned_mask = (
        card_df[CA_RETURNED].isna() |
        card_df[CA_RETURNED].astype(str).str.strip().str.upper().isin(
            ["B", "NO", "NOT RETURNED", "NAN", ""]
        )
    )
    ...
    matched_count   = int((report_df[VR_CARD] != "").sum())
